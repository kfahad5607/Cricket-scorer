...
const BatsmenTable = () => {
    ...
    const { toss, optedTo, homeTeamBatScorecardArray, homeTeamPlayer, awayTeamPlayer, awayTeamBatScorecardArray, firstInningInfo, overs, firstInning, teamTotal, teamWicket, oversBowled, ballCount, secondInning, isMatchFinished } = cricketContext;
                        ...
                        <Row>
                            ...
                            <Col lg='auto' md='auto' sm='auto' xs='auto'><span className='text-1'>{`(${oversBowled} Overs,
                                 CRR: ${(((teamTotal / (ballCount)) * 6).toFixed(2)) > 36 ? '36.00+'
                                    : isNaN(((teamTotal / (ballCount)) * 6).toFixed(2)) ? '0.00'
                                        : ((teamTotal / (ballCount)) * 6).toFixed(2)},
                                            RRR: ${isMatchFinished ? '0.00' : (((((firstInningInfo.totalRunScored + 1 - teamTotal) / ((overs * 6) - ballCount)) * 6) > 36) ? '36.00+' : (isNaN((((firstInningInfo.totalRunScored + 1 - teamTotal) / ((overs * 6) - ballCount)) * 6).toFixed(2))
                                    ? '0.00' : (((firstInningInfo.totalRunScored + 1 - teamTotal) / ((overs * 6) - ballCount)) * 6).toFixed(2)))}
                                )`}</span></Col>
                        </Row>
                    </Card.Header>
                    <Card.Header>
